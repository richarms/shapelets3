...
def chi2Func(params,nmax,im,nm,order=['beta0','beta1','phi','yc','xc'],set_beta=[None,None],set_phi=None,set_xc=[None,None],xx=None,yy=None):
    ...
    if fitParams['xc'] or xx is None:
        ...
        ry=np.array(list(range(0,size[0])),dtype=float)-yc
        rx=np.array(list(range(0,size[1])),dtype=float)-xc
...
if __name__ == "__main__":
    ...
    try:
        beta0,phi0=initBetaPhi(subim,mode='fit')
        xc=img.maxPos(subim)
        rx=np.array(list(range(0,subim.shape[0])),dtype=float)-xc[0]
        ry=np.array(list(range(0,subim.shape[1])),dtype=float)-xc[1]
