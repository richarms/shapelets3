...
if __name__ == '__main__':
    ...
    if opts.init_beta==None:
        ...
        beta0=list(map(float,opts.init_beta.split(',')))
        if len(beta0)==1:
    ...
    if opts.init_xc==None:
        ...
        xc=list(map(float,opts.init_xc.split(',')))
    ...
    if opts.mode.startswith('pol'):
        ...
        if show_plots:
            ...
            plt.subplot(222)
            plt.title('Model')
            ry=np.array(list(range(0,im.shape[0])),dtype=float)-xc1[0]
            rx=np.array(list(range(0,im.shape[1])),dtype=float)-xc1[1]
